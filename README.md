...
| ✅ | `graph` | 图算法（邻接表、BFS/DFS、Dijkstra、拓扑排序、连通分量） |
| ✅ | `string` | 字符串算法（前缀函数、Z 函数、KMP、最长公共子串/子序列） |
| ✅ | `math` | 数论算法（GCD/LCM、整数与模幂、素性判定、埃氏筛、扩展欧几里得、模逆元） |
| ✅ | `dp` | 动态规划模板（最长递增子序列、编辑距离、0/1 背包、硬币找零） |
...
| ✅ | `Text` | 富文本片段：带样式拼接、渲染、UTF-8 显示宽度（正确处理 CJK 全角） |
| ✅ | `Console` | 终端输出封装：TTY 检测、宽度探测、NO_COLOR 约定、彩色自动降级为纯文本 |
| ✅ | `Rule` | 水平分隔线（可带居中标题、自定义填充字符与样式） |

**组件 / 控件**

| 状态 | 组件 | 说明 |
|------|------|------|
| ✅ | `Table` | 表格：表头、列对齐、自动列宽（CJK 对齐），多种边框风格（方角/圆角/双线/ASCII） |
| ✅ | `Panel` | 带边框与标题的面板（标题对齐、内边距、边框/标题着色、固定宽度） |
| ✅ | `Tree` | 树形结构渲染（├─└─│ 连接线、逐结点样式） |
| ✅ | `ProgressBar` | 进度条（百分比、自定义填充/空白字符与样式） |
| ✅ | `Spinner` | 加载动画（盲文/直线/箭头/时钟等预设帧集） |
| ✅ | `Markdown` | 简易 Markdown 渲染（标题/列表/引用/分隔线/代码块/行内强调与代码） |

**布局 / 绘图 / 交互**

| 状态 | 组件 | 说明 |
|------|------|------|
| ✅ | `Layout` | 盒式布局：hbox/vbox 拼接多行块，宽度计算跳过 ANSI（可并排带色面板） |
| ✅ | `Canvas` | 字符画布（点/线/矩形，Bresenham）+ 盲文（Braille）子像素画布（2×4，画折线图） |
| ✅ | `Live` | 可刷新的实时区域：原地重绘（光标回退 + 清屏），非终端自动降级 |
| ✅ | `Prompt` | 交互式输入：文本 / 确认（y-n）/ 单选，输入输出流可注入 |
